...
trait MongodbService[+T] {
   ...
   *   The updated object if operation was successful
   */
  final def updateOne[TT >: T](query: Document, obj: Document)(implicit
      encoder: Encoder[TT],
      decoder: Decoder[TT]
  ): Task[Option[TT]] =
    for {
      coll  <- collection
      value <- coll.findOneAndUpdate(query, obj, FindOneAndUpdateOptions()) *> get[TT](query)
    } yield value
   ...
   *   The updated object if operation was successful
   */
  final def updateMany[TT >: T](query: Document, obj: Document)(implicit
      encoder: Encoder[TT],
      decoder: Decoder[TT]
  ): Task[Option[TT]] =
    for {
      coll  <- collection
      value <- coll.updateMany(query, obj, UpdateOptions(upsert = true)) *> get[TT](query)
    } yield value
  ...
  final def get[TT >: T](
    ...
    value <- ZIO
      ...
      .foldZIO(_ => ZIO.succeed(None), doc => toObject[TT](doc).map(value => Some(value)))
  } yield value
  ...
  final def getMany[TT >: T](
    ...
    result <- coll
      ...
      .limit(Int.MaxValue)
      .all
      .flatMap(docs => toObjects[TT](docs.toSeq))
  } yield result.toList
}
...
object MongodbService {
  ...
  implicit def toObject[T: Encoder: Decoder](doc: Document): Task[T] =
  ...
  implicit def toObjects[T: Encoder: Decoder](objects: Seq[Document]): Task[Seq[T]] =
    ZIO.collectAll(objects.map(toObject[T]))
}
